...
zend_class_entry *monapi_image_resizeoptions_resizeoptionabstract_ce;
zend_class_entry *monapi_image_processor_jpegprocessor_ce;
zend_class_entry *monapi_image_resizeoptions_auto_ce;
zend_class_entry *monapi_image_resizeoptions_exact_ce;
zend_class_entry *monapi_image_thumbnail_ce;

ZEND_DECLARE_MODULE_GLOBALS(monapi)
...
static PHP_MINIT_FUNCTION(monapi)
{
#if PHP_VERSION_ID < 50500
	...
	ZEPHIR_INIT(Monapi_Image_ResizeOptions_ResizeOptionAbstract);
	ZEPHIR_INIT(Monapi_Image_Processor_JPegProcessor);
	ZEPHIR_INIT(Monapi_Image_ResizeOptions_Auto);
	ZEPHIR_INIT(Monapi_Image_ResizeOptions_Exact);
	ZEPHIR_INIT(Monapi_Image_Thumbnail);
...
#ifndef ZEPHIR_RELEASE
static PHP_MSHUTDOWN_FUNCTION(monapi)
{
 ...
 */
static void php_zephir_init_globals(zend_monapi_globals *zephir_globals TSRMLS_DC)
{
...
static PHP_RINIT_FUNCTION(monapi)
{

	zend_monapi_globals *zephir_globals_ptr = ZEPHIR_VGLOBAL;
...
static PHP_RSHUTDOWN_FUNCTION(monapi)
{
...
static PHP_MINFO_FUNCTION(monapi)
{
	php_info_print_box_start(0);
	php_printf("%s", PHP_MONAPI_DESCRIPTION);
	php_info_print_box_end();

	php_info_print_table_start();
	php_info_print_table_header(2, PHP_MONAPI_NAME, "enabled");
	php_info_print_table_row(2, "Author", PHP_MONAPI_AUTHOR);
	php_info_print_table_row(2, "Version", PHP_MONAPI_VERSION);
	php_info_print_table_row(2, "Powered by Zephir", "Version " PHP_MONAPI_ZEPVERSION);
	php_info_print_table_end();


}

static PHP_GINIT_FUNCTION(monapi)
{
	php_zephir_init_globals(monapi_globals TSRMLS_CC);
}

static PHP_GSHUTDOWN_FUNCTION(monapi)
{

}

zend_module_entry monapi_module_entry = {
	...
	PHP_MINIT(monapi),
#ifndef ZEPHIR_RELEASE
	PHP_MSHUTDOWN(monapi),
#else
	NULL,
#endif
	PHP_RINIT(monapi),
	PHP_RSHUTDOWN(monapi),
	PHP_MINFO(monapi),
	PHP_MONAPI_VERSION,
	ZEND_MODULE_GLOBALS(monapi),
	PHP_GINIT(monapi),
	PHP_GSHUTDOWN(monapi),
	NULL,
	STANDARD_MODULE_PROPERTIES_EX
};

#ifdef COMPILE_DL_MONAPI
ZEND_GET_MODULE(monapi)
#endif
